...
int main(void)
{
    ...
    const struct damemory_properties dmp = {
        ...
        DAMEMORY_TAILLATENCY_VERYLOW,      DAMEMORY_PERSISTENCY_PERSISTENT};
    ...
    if (damemory_anchor(1)) {
        perror("damemory_anchor() failed");
        return 1;
    }
    ...
    printf("Write\n");
    WRITEMEM(ptr, 0, NPAGES / 2, 'B');

    printf("Write\n");
    WRITEMEM(ptr, 0, NPAGES / 4, 'C');

    damemory_persist(dh, 0, MEMSIZE);
